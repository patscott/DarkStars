...
c=======================================================================
     ...
     &  sigv,mx,flx,sigsip,sigsdp,tmp1,tmp2
      real*8 dssigmav,dsntmuonyield
      integer istat,itmp,rtype,rtdiff
...
c **************************************************************
     ...
     &    .or.ntcalcmet.eq.4) then ! jkg and/or gould w/ Gauss or full dist.
          call dsntannrate(mx,sigsip,sigsdp,sigv,
     &       arateea,aratesu)

        elseif (ntcalcmet.eq.3) then ! jkg for sun, gould+dk for earth
          call dsntdkannrate(mx,sigsip,sigsdp,sigv,arateea,
     &      aratesu)
        else
...
c...arateea and aratesu in units of 10^24 yr^-1
        ...
        flx=dsntmuonyield(emu,theta,'su',rtdiff,istat)
...
c...one more m^-1 -> km^-1 still to go for rtype=2
        ...
        itmp=istat
        flx=dsntmuonyield(emu,theta,'ea',rtdiff,istat)
c...flx is in units of 10^-30 m^-2(3).
        rateea=flx*arateea
c...we now have units km^-2 yr^-1 gev^-1 deg^-1 if rtype=3.
c...one more m^-1 -> km^-1 still to go for rtype=2
