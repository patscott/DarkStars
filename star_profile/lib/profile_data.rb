...
class HR_History_Data
    ...
    attr_accessor :model_Number, :star_Age, :star_Mass, :star_Age_String
    attr_accessor :model_numbers, :age, :mass, :log_Luminosity, :log_surface_Temp, :log_center_Density, :log_center_Temp
    ...
    def read_history(filename, mod_num)
        ...
        while keep_trying > 0
            begin
                Dvector.read(filename, @dest)
                num = @model_numbers.size
                @dest.each { |ary| raise "Error" if ary.size != num }
                keep_trying = 0
            rescue
                if keep_trying == 0
        ...
        k = @model_numbers.where_first_ge(mod_num)
        if k != nil
            @age = @age[0..k]
            @mass = @mass[0..k]
        ...
        if @star_Age > 1e9
            @star_Age_String = sprintf("%0.2f (Gyr)", @star_Age * 1e-9)
        elsif @star_Age > 1e6
            @star_Age_String = sprintf("%0.2f (Myr)", @star_Age * 1e-6)
        else
            @star_Age_String = sprintf("%0.2f (Kyr)", @star_Age * 1e-3)
        end
    end
...
class EosData
    ...
    attr_accessor :opacity_EoS, :entropy_EoS, :ionization_EoS, :psi_EoS
    attr_accessor :pressure_EoS, :energy_EoS, :eos_logTs, :eos_logRHOs
    attr_accessor :eos_data_ylen, :eos_ymax, :eos_ymin
    attr_accessor :eos_data_xlen, :eos_xmax, :eos_xmin
    attr_accessor :psi5


    def initialize
        @dir = '../star_profile/EOS_DATA'
        read_eos_logTs
        read_eos_logRHOs
        @opacity_EoS = read_eos_data('Opacity_EoS')
        @entropy_EoS = read_eos_data('Entropy_EoS')
        @ionization_EoS = read_eos_data('Ionization_EoS')
        @pressure_EoS = read_eos_data('Pressure_EoS')
        @energy_EoS = read_eos_data('Energy_EoS')
        @psi_EoS = read_eos_data('PSI_EoS')
        Dvector.read('star_data/psi5.data', @psi5 = [Dvector.new, Dvector.new], 2)
    end
    
    def read_eos_logTs
        data = Dvector.read(@dir + '/logTs_for_EoS.data')
        @eos_logTs = data[0]
        @eos_data_ylen = @eos_logTs.size
        @eos_ymin = @eos_logTs.min
        @eos_ymax = @eos_logTs.max
    end

    def read_eos_logRHOs
        data = Dvector.read(@dir + '/logRHOs_for_EoS.data')
        @eos_logRHOs = data[0]
        @eos_data_xlen = @eos_logRHOs.size
        @eos_xmin = @eos_logRHOs.min
        @eos_xmax = @eos_logRHOs.max
    end
    
    def read_eos_data(name)
        data = Dtable.new(@eos_data_xlen, @eos_data_ylen)
        data.read(@dir + '/' + name + '.data')
        return data
    end
...
class ProfileData
   ...
   # variables related to other nuclear reactions -- metal burning that doesn't consume helium (ergs/g/sec)
    ...
    @@num_sx_lines = 75
    @@num_wimpy_lines = 5
    @@num_lines = @@num_header_lines + @@num_sx_lines + @@num_wimpy_lines
    
    # extra WIMPy things
    attr_accessor :wimp_dens, :wimp_mfp, :wimp_L_transport, :wimp_E_transport, :wimp_E_annihilation
    ...
    def initialize
        ...
        @wimp_dens = Dvector.new
        @wimp_mfp = Dvector.new
        @wimp_L_transport = Dvector.new
        @wimp_E_transport = Dvector.new
        @wimp_E_annihilation = Dvector.new
    ...
    def unpack_rows(filename)
        ...
        if (sx.size != @@num_sx_lines + @@num_wimpy_lines)
   ...
   # variables related to other nuclear reactions -- metal burning that doesn't consume helium (ergs/g/sec)
    ...
    # WIMPy details
        @wimp_dens = sx[75]
        @wimp_mfp = sx[76]
        @wimp_L_transport = sx[77]
        @wimp_E_transport = sx[78]
        @wimp_E_annihilation = sx[79]
        ...
        if @star_Age > 1e9
            ...
            @star_Age_String = sprintf("%0.3f (Kyr)", @star_Age * 1e-3)
        end
    ...
    def read_profile(filename)
        ...
        while keep_trying > 0
            begin
                ...
                puts "failed trying to read from #{filename}.  sleep and try again later."
