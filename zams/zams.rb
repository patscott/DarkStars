...
class ZAMS_Data
    ...
    attr_accessor :log_Ms, :log_Tcents, :log_KAPPAs, :fraction_CNO, :log_fluxes
    ...
    def initialize(filename)
        ...
        lsun = 3.8e33
        rsun = 7e7
        fluxes = lsun * 10**@log_Ls / (4 * 3.14159 * (rsun * 10**(@log_Rs))**2)
        @log_fluxes = fluxes.safe_log10
    end
...
class ZAMSPlots
    ...
    def initialize
        ...
        @zams_data_array = [
            ...
            @z02_data = ZAMS_Data.new("zams_data/z01.log"),
            @z02_data = ZAMS_Data.new("zams_data/z02.log"),
            @z03_data = ZAMS_Data.new("zams_data/z03.log") ]
        #report(@z02_data.log_RHOs, @z02_data.log_Tcents)
        @num_zams = @zams_data_array.size
        @num_masses = @z02_data.masses.size
        @legends = [ 'Z = 0.0001', 'Z = 0.001', 'Z = 0.004', 'Z = 0.01', 'Z = 0.02', 'Z = 0.03' ]
        @colors = [ RoyalPurple, FireBrick, Lilac, Goldenrod, Coral, BrightBlue ]
        ...
        t.def_figure("ZAMS_plots3") { zams_plots3 }
        t.def_figure("radii") { zams_radii }
    ...
    def zams_fancy
        ...
        t.ylabel = 'Luminosity ($\mathrm{L}_{\odot}$)'
    ...
    def zams_radii
        t.show_plot_with_legend('plot_top_margin' => 0.01, 'legend_top_margin' => 0.15) { radius_plot }
    end
    
    def zams_radii_lars
        t.show_plot_with_legend('plot_top_margin' => 0.01, 'legend_top_margin' => 0.15) { radius_plot_lars }
    end
    ...
    def flux_plot
        zams_plot('log F ($erg/cm^2/sec$)', 22, 14) do
            @zams_data_array.each_index do |i|
                zams = @zams_data_array[i]
                t.show_polyline(zams.log_Ms, zams.log_fluxes, @colors[i], @legends[i])
            end
        end
    end
    
    def radius_plot_lars
        t.xlabel = 'Mass ($M_{\odot}$)'
        t.ylabel = 'R ($R_{\odot}$)'
        xleft = 0.45
        xright = 1.05
        ...
        t.show_plot('left_boundary' => xleft, 'right_boundary' => xright,
            ...
            @zams_data_array.each_index do |i|
                zams = @zams_data_array[i]
                xs = zams.masses
                ys = zams.log_Rs.exp10
                t.show_polyline(xs, ys, @colors[i], @legends[i])
    ...
    def zams_plots3
        t.show_plot_with_legend('plot_top_margin' => 0.01, 'legend_top_margin' => 0.01) { plots3 }
    end
        
    def plots3
        t.rescale(0.9)
        t.show_title('Luminosity, Radius, Flux')
        t.no_title
        num_plots = 3; row = 1
        t.subplot(t.row_margins('num_rows' => num_plots, 'row' => row)) do
            t.xaxis_type = AXIS_WITH_TICKS_ONLY; t.no_xlabel
    ...
    def plots1
        t.rescale(0.9)
        t.show_title('Temperature, Density')
        t.no_title
        num_plots = 3; row = 1
        t.subplot(t.row_margins('num_rows' => num_plots, 'row' => row)) do
            t.xaxis_type = AXIS_WITH_TICKS_ONLY; t.no_xlabel
    ...
    def plots2
        t.rescale(0.9)
        t.ylabel_scale = 0.9
        t.show_title('Power, Opacity, Nuclear Timescale')
        t.no_title
        num_plots = 3; row = 1
        t.subplot(t.row_margins('num_rows' => num_plots, 'row' => row)) do
            t.xaxis_type = AXIS_WITH_TICKS_ONLY; t.no_xlabel
